...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    cancelled: "bg-slate-800/50 text-slate-400 border-slate-700/50",
  };
  const cls = map[status] ?? "bg-slate-800/50 text-slate-400 border-slate-700/50";
  return (
  ...
  return (
    <div className="space-y-6 p-1">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-100">Payment Plans & Financial Control</h2>
          <p className="text-xs text-slate-500">Real-time payment monitoring and risk management</p>
        </div>
      </div>

      {/* Cash Flow Summary SVG Chart */}
      <div className="rounded-2xl bg-slate-900/60 border border-slate-700/50 p-5 shadow-lg">
        <div className="flex items-center gap-2 mb-4">
          <TrendingUp className="w-4 h-4 text-amber-400" />
          <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-widest">Cash Flow Overview</h3>
        </div>

        <div className="space-y-3">
          {/* Collected bar */}
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 w-28 flex-shrink-0">Collected</span>
            <div className="flex-1 h-7 rounded-lg bg-slate-800/60 overflow-hidden relative">
              <div
              ...
              >
                {collectedPct > 15 && (
                  <span className="text-xs font-bold text-emerald-100">{collectedPct}%</span>
                )}
              </div>
              {collectedPct <= 15 && (
            ...
            <span className="text-xs font-bold text-emerald-300 w-24 text-right flex-shrink-0">
          ...
          {/* Outstanding bar */}
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 w-28 flex-shrink-0">Outstanding</span>
            <div className="flex-1 h-7 rounded-lg bg-slate-800/60 overflow-hidden relative">
              <div
                className="h-full bg-gradient-to-r from-red-700 to-red-500 rounded-lg transition-all duration-700 flex items-center pl-3"
                style={{ width: `${100 - collectedPct}%` }}
              >
                {100 - collectedPct > 15 && (
                  <span className="text-xs font-bold text-red-100">{100 - collectedPct}%</span>
                )}
              </div>
              {100 - collectedPct <= 15 && (
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-red-400">
                  {100 - collectedPct}%
                </span>
              )}
            </div>
            <span className="text-xs font-bold text-red-300 w-24 text-right flex-shrink-0">
        ...
        <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-slate-200">£{grandTotal.toLocaleString()}</span>
        </div>
      </div>
      ...
      {(highRisk.length > 0 || overdue.length > 0) && (
        <div className="space-y-3">
          ...
          {highRisk.map((r) => (
            <div
              key={`hr-${r.id}`}
              className="flex items-center justify-between rounded-xl bg-red-950/40 border border-red-800/40 px-4 py-3"
            >
              <div className="flex items-center gap-3">
                <span className="text-lg">🔴</span>
                <div>
                  <p className="text-sm font-semibold text-red-200">HIGH RISK: {r.clientName}</p>
          ...
          {overdue.map((r) => (
            <div
              key={`od-${r.id}`}
              className="flex items-center justify-between rounded-xl bg-orange-950/40 border border-orange-800/40 px-4 py-3"
            >
              <div className="flex items-center gap-3">
                <span className="text-lg">🟠</span>
                <div>
                  <p className="text-sm font-semibold text-orange-200">OVERDUE: {r.clientName}</p>
            ...
            {topRisk.map((r) => (
              <div key={`nudge-${r.id}`} className="flex items-center justify-between px-5 py-3 gap-4">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-100 truncate">{r.clientName}</p>
                  <p className="text-xs text-slate-500">{r.bookingRef} · £{r.outstanding.toLocaleString()} outstanding</p>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button
                    onClick={() => handleReminder(r.clientName)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-900/30 border border-amber-700/40 text-amber-300 hover:bg-amber-800/40 text-xs font-semibold transition-all duration-200"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-900/30 border border-blue-700/40 text-blue-300 hover:bg-blue-800/40 text-xs font-semibold transition-all duration-200"
                  >
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-700/50 overflow-hidden shadow-lg">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-xs font-semibold capitalize transition-all duration-200 ${
                  sortKey === k
                    ...
                    : "bg-slate-800/60 border border-slate-700/50 text-slate-500 hover:text-slate-300"
                }`}
              >
        ...
        {sorted.length === 0 ? (
          <p className="text-sm text-slate-500 text-center py-10">No bookings found</p>
        ) : (
          <div className="divide-y divide-slate-800/60 max-h-[600px] overflow-y-auto">
            {sorted.map((r) => (
              <div key={r.id} className="px-5 py-4 hover:bg-slate-800/30 transition-colors">
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {r.isHighRisk && (
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-red-900/50 border border-red-700/50 text-red-300">
                          HIGH RISK
                        </span>
                      )}
                      {r.isOverdue && (
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-orange-900/50 border border-orange-700/50 text-orange-300">
                          OVERDUE
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-1">
                      <div className="flex-1 h-2 rounded-full bg-slate-700/60 overflow-hidden max-w-xs">
                        <div
                      ...
                      <span className="text-xs text-slate-400 flex-shrink-0">
                        ...
                        <span className="ml-1.5 text-slate-500">({r.progress}%)</span>
                      </span>
                    </div>
                    {r.outstanding > 0 && r.status !== "cancelled" && (
                      <p className="text-xs text-red-400">£{r.outstanding.toLocaleString()} outstanding</p>
                    )}
                  </div>
                  ...
                  {r.status !== "cancelled" && r.outstanding > 0 && (
                    <div className="flex flex-col gap-2 flex-shrink-0">
                      <button
                        onClick={() => handleReminder(r.clientName)}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700/60 hover:bg-amber-900/30 hover:border-amber-700/40 text-xs font-semibold text-slate-300 hover:text-amber-300 transition-all duration-200 whitespace-nowrap"
                      >
                        <Send className="w-3 h-3" />
                        Send Reminder
                      </button>
                      <button
                        onClick={() => handleSplitPayment(r.clientName)}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700/60 hover:bg-blue-900/30 hover:border-blue-700/40 text-xs font-semibold text-slate-300 hover:text-blue-300 transition-all duration-200 whitespace-nowrap"
                      >
                        <CreditCard className="w-3 h-3" />
                        Offer Split Payment
                      </button>
