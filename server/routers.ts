...
Include 6-8 attractions, 4-5 dining experiences, exactly 3 accommodation tiers, 5 insider tips, and 7 itinerary days. All specific to ${input.destination}.`;
  ...
  // ─── Social Hub (AI-powered weekly post generation) ────────────────────────
  socialHub: router({
    generateWeek: adminMiddleware
      .input(z.object({
        platforms: z.array(z.enum(['facebook', 'instagram', 'twitter', 'tiktok', 'linkedin'])).default(['facebook']),
      }))
      .mutation(async ({ input, ctx }) => {
        const userId = (ctx as any).user?.id;
        if (!userId) throw new TRPCError({ code: 'UNAUTHORIZED' });
        ...
        try {
          ...
          const db = getDb();
          const destinations = await db.query.destinationGuides.findMany({ limit: 5 });
          const bookings = await db.query.bookings.findMany({ limit: 5 });
          const testimonials = await db.query.testimonials.findMany({ limit: 3 });

          const businessContext = `
Recent Destinations: ${destinations.map((d: any) => d.destination).join(', ')}
Upcoming Bookings: ${bookings.length} clients booked
Client Testimonials: ${testimonials.length} happy customers
          `.trim();

          const postConfigs = [
            { day: 'Monday', type: 'Travel Hacks', desc: 'Money-saving travel tips and insider secrets' },
            { day: 'Tuesday', type: 'Destination Spotlight', desc: 'Feature a unique destination with insider tips' },
            { day: 'Wednesday', type: 'Customer Story', desc: 'Showcase a client testimonial or success story' },
            { day: 'Thursday', type: 'Deal Alert', desc: 'Highlight a current promotion or special offer' },
            { day: 'Friday', type: 'Featured Destination', desc: 'In-depth guide to an exotic destination' },
            { day: 'Saturday', type: 'Culture & Philosophy', desc: 'Thought leadership on luxury travel' },
            { day: 'Sunday', type: 'Week Recap & CTA', desc: 'Recap the week and encourage bookings' },
          ];

          const posts = [];

          for (const config of postConfigs) {
            const prompt = `Create a premium Facebook post for luxury travel brand CB Travel.
...
Requirements:
- Write as if from CB Travel's voice (luxury, aspirational, warm)
- Include 3-4 relevant emojis
- Be concise and engaging
- Format your response EXACTLY like this:

[TITLE]
The main headline (under 80 characters)

[CONTENT]
Main post body (2-3 sentences, under 200 characters)

[CAPTION]
Short caption for Instagram/Twitter (under 100 characters)

[HASHTAGS]
#LuxuryTravel #CBTravel #TravelConcierge #ExoticDestinations #WanderlustAddict

[IMAGE_PROMPT]
A detailed DALL-E prompt for a stunning image that complements this post`;
            ...
            const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: 'system',
                    content: 'You are a luxury travel brand social media expert. Create premium, engaging posts that showcase exotic destinations and travel experiences. Always include relevant emojis and use a warm, aspirational tone.',
                  },
                  {
                    role: 'user',
                    content: prompt,
            ...
            const imagePromptMatch = content.match(/\\[IMAGE_PROMPT\\]\\s*\\n([^\\[]+?)$/s);
          ...
          console.error('[SocialHub] generateWeek error:', error);
          throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to generate posts' });
        }
      }),
  }),
